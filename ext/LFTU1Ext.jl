...
# import LFTsAnaTools: sign, correlator_fit_function, get_symmetry
...
#     end
#     ypi0 = 2*(uwhist[1,1,1,:] + uwhist[1,2,2,:]) - (uwhist[2,1,1,:] + uwhist[2,2,2,:]) + 2*uwhist[2,1,2,:]
#     # uwDD = make_disconnected_pieces(corrws) # this does not seem to affect at
#     # all...
#     # ypi0 = ypi0 + uwrealsym(uwDD[1,1,:]) + uwrealsym(uwDD[2,2,:]) - 2*uwrealsym(uwDD[1,2,:])
#     corrws.ydata = ypi0
#     return nothing
# end
